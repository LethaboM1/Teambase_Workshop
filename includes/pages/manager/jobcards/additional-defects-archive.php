			...
			$jscript .= "
									...
									$('#searchBtn').click(function () {
										$.ajax({
											...
											data: {
												cmd:'search',
												type: 'additional-defect-report-archive',
												search: $('#search').val()
											},
...
<div class="row">
	<table class="table table-hover">
		...
		<tbody id='jobcard_list'>
			...
			$get_defect_reports = dbq("select * from jobcard_reports where reviewed=0 limit {$start},$lines");


			if (dbr($get_defect_reports) > 0) {
				while ($row = dbf($get_defect_reports)) {
					require "./includes/pages/manager/jobcards/list_archive_defect_report.php";
				}
			}
