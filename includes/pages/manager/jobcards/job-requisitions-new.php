...
<div class="row">
	...
	<div id="job_requisitions_list" class="col-xl-12">
		...
		if ($_SESSION['user']['role'] == 'clerk') {
			$get_requisitions = dbq("select * from jobcard_requisitions where status='requested' and clerk_id={$_SESSION['user']['user_id']} order by datetime");
		} else if ($_SESSION['user']['role'] == 'buyer') {
			$get_requisitions = dbq("select * from jobcard_requisitions where status='requested' and buyer_id={$_SESSION['user']['user_id']} order by datetime");
		} else {
			$get_requisitions = dbq("select * from jobcard_requisitions where status='requested' order by datetime");
		}
